...
const catalog: Capability[] = [
  ...
  { id: "tool.figma", name: "Figma", description: "Design inspection and component/design-system integration.", kind: "tool", provider: "Figma", version: "1.0.0", status: "available", permissions: ["design.read"], dependencies: [], tags: ["design", "ui"] },
  ...
  { id: "integration.strengthcode", name: "StrengthCode", description: "Specialized training capability provider.", kind: "integration", provider: "StrengthCode", version: "1.0.0", status: "planned", permissions: [], dependencies: [], tags: ["specialized"] },
...
export function listCapabilities(): Capability[] { return [...catalog]; }
...
export function resolveCapabilities(requested: string[]): CapabilityResolution {
  ...
  const visit = (id: string) => {
    ...
    if (!capability) { missing.push(id); return; }
    if (capability.status !== "available") { unavailable.push(id); return; }
    for (const dependency of capability.dependencies) visit(dependency.id);
    resolved.push(capability);
  };

  requested.forEach(visit);
  return { requested, resolved, missing, unavailable };
}
