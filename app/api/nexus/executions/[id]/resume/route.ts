...
export async function POST(
  ...
  if (authRequired()) {
    ...
    projectId = process.env.RESONANCE_PROJECT_ID ?? null;
  }
  ...
  // Re-verify auth against intent.projectId to prevent cross-project writes
  if (authRequired() && intent.projectId !== projectId) {
      await db.from("nexus_execution_requests").update({ status: "failed", updated_at: new Date().toISOString() }).eq("project_id", projectId).eq("idempotency_key", existing.idempotency_key);
      return NextResponse.json({ error: "Cross-project write attempt detected." }, { status: 403 });
  }
