...
export async function POST(request: Request) {
  ...
  if (db) {
    const { data: admission, error: admissionError } = await db.rpc("claim_nexus_execution_request", {
      p_project_id: intent.projectId,
      p_idempotency_key: idempotencyKey,
      p_request_hash: hash,
      p_window_start: new Date(Date.now() - RATE_LIMIT_WINDOW_MS).toISOString(),
      p_limit: RATE_LIMIT_MAX_REQUESTS,
    });
    if (admissionError) return NextResponse.json({ error: admissionError.message }, { status: 500 });
    const result = Array.isArray(admission) ? admission[0] : admission;
    if (!result) return NextResponse.json({ error: "Execution admission returned no result." }, { status: 500 });
    if (result.conflict) return NextResponse.json({ error: "Idempotency-Key was already used for a different execution request." }, { status: 409 });
    if (result.replay) return NextResponse.json(result.response ?? { status: result.status, executionId: result.execution_id }, { status: result.response ? 200 : 202, headers: { "X-Idempotent-Replay": "true" } });
    if (!result.accepted && result.status === "rate_limited") return NextResponse.json({ error: "Execution rate limit exceeded. Retry after the current window." }, { status: 429, headers: { "Retry-After": "60" } });
  }
