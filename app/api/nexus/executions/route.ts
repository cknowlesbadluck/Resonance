...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (existing) return NextResponse.json({ execution: existing, idempotentReplay: true }, { status: 200 });
    ...
    const evidenceSink = { recordEvidence: async (item: Parameters<typeof persistence.saveEvidence>[0]) => persistence.saveEvidence(item, auth.projectId) };
    const result = await new NexusExecutor(nexusAdapters, evidenceSink).execute(plan);
    await persistence.saveExecution(result.execution, auth.projectId, idempotencyKey);
