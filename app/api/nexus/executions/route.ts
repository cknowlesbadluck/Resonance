...
const sink = {
  ...
  recordEvent: async (event: NexusEvent) => {
    ...
    const { error } = await db.from("events").upsert({ id: event.id, project_id: event.projectId, source: event.source, type: event.type, status: event.status, correlation_id: event.correlationId, actor_id: event.actorId ?? null, resource_type: "execution", resource_id: event.resourceId ?? null, external_id: `${event.correlationId}:${event.type}`, payload: event.payload ?? {}, created_at: event.createdAt, updated_at: event.createdAt }, { onConflict: "project_id,source,external_id" });
...
export async function GET(request: Request) {
  ...
  if (authRequired() && !(await authenticateNexusRequest(request, projectId))) return NextResponse.json({ error: "Authentication or project authorization required." }, { status: 401 });
...
export async function POST(request: Request) {
  ...
  try { body = await readJson(request); } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Invalid JSON body." }, { status: 400 }); }
  ...
  const intent: NexusIntent = { id: body.id ?? crypto.randomUUID(), projectId, objective: body.objective.trim(), requestedBy: actorId, requirements: body.requirements, contextRefs: body.contextRefs ?? [], metadata: body.metadata ?? {} };
  ...
  try {
    ...
    const result = await new NexusExecutor(nexusAdapters, { recordEvidence: async (item) => sink.recordEvidence(item, intent.projectId), recordExecution: async (execution) => sink.recordExecution(execution, intent.projectId), recordEvent: sink.recordEvent }).execute(plan);
