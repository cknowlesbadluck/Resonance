...
export async function POST(request: Request) {
  ...
  if (!projectId || typeof projectId !== "string" || !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(projectId)) {
    return NextResponse.json({ error: "Valid projectId UUID is required." }, { status: 400 });
  }
