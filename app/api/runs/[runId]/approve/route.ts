...
export async function POST(request: Request, context: { params: Promise<{ runId: string }> }) {
  try {
    ...
    if (role !== "owner" && role !== "admin") return NextResponse.json({ error: "Only project owners and admins can approve runs" }, { status: 403 });
    ...
    const blocked = definition.steps.slice(currentIndex + 1).find((step) => !capabilityAllowed(role, step.capability));
    if (blocked) return NextResponse.json({ error: "Permission denied after approval gate", step_id: blocked.id }, { status: 403 });
