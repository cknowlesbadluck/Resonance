...
2. Owner sets `SUPABASE_SERVICE_ROLE_KEY` on resonancenexus only. Exit: `/api/ready` 200. Confirmed missing at 11:06. Env list still has URL/anon/auth/project id/deploy stage only.
3. Durable `github.repository.read` evidence with deny proofs. Needs `GITHUB_TOKEN` + `GITHUB_WEBHOOK_SECRET`.
4. Quicksilver device HG CHR-55 on iPhone 16e from `871706d9` or later. Simulator CI is not acceptance.
...
6. Conduit freeze; repair #119/#120 off current main (`102c5387`). Do not merge red.
